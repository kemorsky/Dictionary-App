...
const server = setupServer(
  // prepare the mock server I will be working with
        ...
        phonetics: [
          {
            text: "/ˈpʌmpkɪn/",
            audio:
              "https://api.dictionaryapi.dev/media/pronunciations/en/pumpkin-us.mp3",
          },
        ],
        meanings: [
          {
...
test("should render a searched word", async () => {
  ...
  expect(
    await within(resultsSection).findByText("pumpkin")
  ).toBeInTheDocument();
});
...
test("it should let me remove words from favorites", async () => {
  ...
  const removeButton = within(favoriteContainer).getByRole("button", {
    name: /remove/i,
  });
...
test("it should let me play the audio file of the word", async () => {
  ...
  // wait for the word to appear
  const wordHeader = await screen.findByRole("heading", {
    name: /word: pumpkin/i,
  });
  expect(wordHeader).toBeInTheDocument();
